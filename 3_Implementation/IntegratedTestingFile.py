# pylint: disable=C0301,C0114,C0116,C0121,C0103
import sqlite3
...
def test_add():
    ...
    assert HospitalDetailsAdd.hospital_add("12345678", "dasdssadasdsdasdassdsa", "1999-11-19", "100", "9004088473",
                                           "BKMS@gmail.com",
...
# to add doctors
def test_add_doctor():
    assert docregister.add_doctor(65412337, "riya", "cough", 12345678, "acd", 9632145808, 365412012369,
                                  "12345") == success
    assert docregister.add_doctor(65412333, "vikas", "fever", 12345678, "acd", 9632145801, 365412012363,
                                  "12345") == success
    assert docregister.add_doctor(65412334, "ajay", "cold", 12345678, "acd", 9632145802, 365412012364,
                                  "12345") == success
    assert docregister.add_doctor(65412335, "swaroop", "detaist", 12345678, "acd", 9632145804, 365412012365,
                                  "12345") == success
    assert docregister.add_doctor(65412336, "sai", "surgery", 12345678, "acd", 9632145805, 365412012366,
                                  "12345") == success
    # testing by giving non-existed hospital_id
    assert docregister.add_doctor(65412336, "sai", "surgery", 12345666, "acd", 9632145805, 365412012366,
                                  "12345") == fail
    # testing length of diff variables
    assert docregister.add_doctor(654123, "riya", "cough", 123456, "acd", 9632145877946, 36541201236, "12345") == fail
    assert docregister.add_doctor(65474674, "vikas", "fever", 156867745, "acd", 9632145801, 36541201236374,
                                  "12345") == fail


# to check whether doctors are resisted or not
def test_check_id():
...
# to drop added doctors
def test_drop_doctor():
    assert docregister.drop_doctor(65412337) == success
    assert docregister.drop_doctor(65412333) == success
    assert docregister.drop_doctor(65412334) == success
    assert docregister.drop_doctor(65412335) == success
    assert docregister.drop_doctor(65412336) == success
    # testing to drop when doctor is not existed
    assert docregister.drop_doctor(125412335) == fail
    assert docregister.drop_doctor(65486512336) == fail


# to check whether view doctors is working fine

def test_view_doctors():
    assert docregister.view_doctor() == 1
